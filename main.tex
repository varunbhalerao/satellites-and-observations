...
\shorttitle{The effect of artificial satellites on ground-based observational astronomy}
\shortauthors{Astronomers et al.}
%%
...
\title{The effect of artificial satellites on ground-based observational astronomy}
...
\correspondingauthor{An Astronomer}
\email{curious@astro.nomer}

\author[0000-0000-0000-0000]{An Astronomer}
\affil{Earth}
...
\begin{abstract}
There is a growing concern among astronomers that clusters of satellites will ruin the night sky for ground-based observations. In this work, we try to quantify and evaluate the impact of small, medium and large satellite clusters on ground-based observational astrophysics.
\end{abstract}
...
Space is becoming easier to access, and satellites are being used for more and more purposes. There are currently about five thousand artificial satellites orbiting the earth\footnote{See ESA stats at \url{https://www.esa.int/Our_Activities/Space_Safety/Space_Debris/Space_debris_by_the_numbers}}. While they occasionally impact observations, the net effect is negligible.

The aim of this whitepaper is to evaluate whether upcoming satellite networks like Starlink, Oneweb etc will change this picture, and to quantify the impact on astronomical observations\footnote{We note that research-centric satellite networks also have been proposed, and we do not intend to vilify space exploration.}

\section{Effect of a single satellite} \label{sec:single}
For a telescope taking a long exposure, a satellite appears as a streak in the image. That has two immediate effects: first, it impacts a much larger part of the image as compared to a point source, and second: the satellite appears fainter as the photons are spread over this larger area. In this section, we attempt to quantify both these effects by using representative numbers for satellite magnitudes and telescope capabilities.

\section{Probability of seeing a satellite} \label{sec:odds}
If we assume a large number of satellites with some distribution around the earth, what are the odds that a satellite will pass through a given field during a long exposure?

\section{When and where do we see the effects?} \label{sec:whenwhere}
Are the effects limited to morning and evening twilight? What part of the sky are affected: just low altitudes, or all over?

\section{Putting it together: fraction of observations affected} \label{sec:fraction}
Based on everything we have calculated thus far, in this section we summarize the net impact. What fraction of observations will be affected by satellite trails? How bad will the effect be? Will trails completely ruin images, or will we now be adding a ``satellite removal'' step just like cosmic ray removal?
...
We thank all the people that have contributed to this work with fruitful discussions.
